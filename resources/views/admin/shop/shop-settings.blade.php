    ...
    <div class="bg-gray-100 flex-1 mt-16 p-6 md:mt-16">
        <div class="flex flex-col w-full justify-center">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white">
                                    ...
                                    <div
                                        ...
                                        <div class="relative">
                                            ...
                                            <form action="{{ route('shop-category.store') }}" method="POST"
                                                id="addCategoryProduct" enctype="multipart/form-data">
                                                @csrf
                                                <div class="grid gap-4 mb-4 grid-cols-2 p-4">
                                                    <div class="col-span-2">
                                                        ...
                                                        <input type="text" name="name" id="name"
                                                            ...
                                                            placeholder="Masukkan nama category product" required="">
                                                    </div>
                                                </div>
                                                <button type="submit"
                        ...
                        <tr
                            ...
                            <td class="px-6 py-2 font-medium text-gray-700">
                                <div class="flex flex-row justify-start gap-3 items-center">
                                    ...
                                    <button data-modal-target="edit-category-{{ $cty->id }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        Edit
                                    </button>

                                    <!-- Edit Modal -->
                                    <div id="edit-category-{{ $cty->id }}" tabindex="-1" aria-hidden="true"
                                        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 left-0 right-0 bottom-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                                        <div
                                            class="relative p-4 w-full max-w-md bg-white rounded-lg shadow-lg dark:bg-gray-700">
                                            <!-- Modal content -->
                                            <div class="relative">
                                                ...
                                                <div
                                                    ...
                                                    <button type="button"
                                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                        data-modal-toggle="edit-category-{{ $cty->id }}">
                                                        <svg class="w-3 h-3" aria-hidden="true"
                                                            xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 14 14">
                                                            <path stroke="currentColor" stroke-linecap="round"
                                                ...
                                                <!-- Modal body -->
                                                <form action="{{ route('shop-category.update', $cty->id) }}" method="POST"
                                                    id="editCategoryProduct-{{ $cty->id }}" enctype="multipart/form-data"
                                                    class="p-4 md:p-5">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                                        <div class="col-span-2">
                                                            <label for="edit-name-{{ $cty->id }}"
                                                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                                                Category</label>
                                                            <input type="text" name="name" id="edit-name-{{ $cty->id }}"
                                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                                                placeholder="Masukkan nama category"
                                                                value="{{ $cty->name }}" required="">
                                                        </div>
                                                    </div>
                                                    <button type="submit"
                                    ...
                                    <form action="{{ route('shop-category.destroy', $cty->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                ...
                <div class="p-3">
                    {{ $type->links('tailwind-pagination') }}
                </div>
                                    ...
                                    <div id="crud-modal" tabindex="-1" aria-hidden="true"
                                        ...
                                        <div
                                            ...
                                            <div class="relative">
                                                ...
                                                <!-- Modal body -->
                                                <form action="{{ route('shop-type.store') }}" method="POST"
                                                    ...
                                                    <div class="grid gap-4 mb-4 grid-cols-2 p-4">
                                                        <div class="col-span-2">
                                                            ...
                                                            <input type="text" name="name" id="name"
                                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                                                placeholder="Masukkan nama type product" required="">
                                                        </div>
                                                    </div>
                                                    <button type="submit"
                                                        class="text-white inline-flex ml-4 items-center bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        ...
                        <tbody>
                            ...
                            <tr
                                ...
                                <td class="px-6 py-2 font-medium text-gray-700">
                                    <div class="flex flex-row justify-start gap-3 items-center">
                                        <!-- Button to toggle Edit Modal -->
                                        <button data-modal-target="edit-modal-{{ $typ->id }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            Edit
                                        </button>

                                        <!-- Edit Modal -->
                                        <div id="edit-modal-{{ $typ->id }}" tabindex="-1" aria-hidden="true"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 left-0 right-0 bottom-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                                            <div
                                                class="relative p-4 w-full max-w-md bg-white rounded-lg shadow-lg dark:bg-gray-700">
                                                <!-- Modal content -->
                                                <div class="relative">
                                                    ...
                                                    <div
                                                        ...
                                                        <button type="button"
                                                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-toggle="edit-modal-{{ $typ->id }}">
                                                            <svg class="w-3 h-3" aria-hidden="true"
                                                                xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                viewBox="0 0 14 14">
                                                                <path stroke="currentColor" stroke-linecap="round"
                                                    ...
                                                    <!-- Modal body -->
                                                    <form action="{{ route('shop-type.update', $typ->id) }}"
                                                        method="POST" id="editCategoryForm-{{ $typ->id }}"
                                                        enctype="multipart/form-data" class="p-4 md:p-5">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="grid gap-4 mb-4 grid-cols-2">
                                                            <div class="col-span-2">
                                                                <label for="edit-name-{{ $typ->id }}"
                                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                                                    Category</label>
                                        ...
                                        <form action="{{ route('shop-type.destroy', $typ->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
...
document.addEventListener('DOMContentLoaded', function() {
    // Mengatur event listener untuk tombol toggle modal create
    const modalCategoryCreate = document.querySelectorAll('[data-modal-toggle="category-modal"]');
    const modalTargetCategory = document.getElementById('category-modal');

    modalCategoryCreate.forEach(toggle => {
        toggle.addEventListener('click', () => {
            modalTargetCategory.classList.toggle('hidden');
            if (!modalTargetCategory.classList.contains('hidden')) {
                document.body.classList.add('overflow-hidden');
            } else {
                document.body.classList.remove('overflow-hidden');
            }
    ...
    function showEditCategory(categoryId) {
        var modal = document.getElementById('edit-category-' + categoryId);
        modal.classList.remove('hidden');
        modal.setAttribute('aria-hidden', 'false');
        document.body.classList.add('overflow-hidden');
    }
    ...
    // Fungsi untuk menutup modal edit
    function closeEditCategory(categoryId) {
        var modal = document.getElementById('edit-category-' + categoryId);
        modal.classList.add('hidden');
        modal.setAttribute('aria-hidden', 'true');
        document.body.classList.remove('overflow-hidden');
    ...
    // Menambahkan event listener pada tombol Edit untuk masing-masing kategori
    var editCategoryButtons = document.querySelectorAll('[data-modal-target^="edit-category"]');
    ...
    // Menambahkan event listener pada tombol Close di dalam modal edit
    var closeEditCategoryButtons = document.querySelectorAll('[data-modal-toggle^="edit-category"]');
    closeEditCategoryButtons.forEach(function(button) {
        var categoryId = button.getAttribute('data-modal-toggle').split('-').pop();
        button.addEventListener('click', function() {
    ...
    // Menangani submit form pada modal edit
    var editCategoryForms = document.querySelectorAll('form[id^="editCategoryProduct"]');
    editCategoryForms.forEach(function(form) {
        form.addEventListener('submit', function(event) {
            event.preventDefault();
            var categoryId = form.getAttribute('id').split('-').pop();
            var formData = new FormData(form);
            ...
            fetch(form.action, {
                ...
                .then(data => {
                    // Handle success response, you may redirect or show a success message
                    console.log(data); // Log response for debugging
                    closeEditCategory(categoryId); // Tutup modal setelah sukses update
                    window.location
                        .reload(); // Reload halaman atau perbarui data tanpa reload jika memungkinkan
                })
                .catch(error => {

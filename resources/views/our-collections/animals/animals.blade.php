    ...
    <div id="default-carousel" class="relative max-w-full md:w-full" data-carousel="slide">
        ...
        <div class="relative h-screen overflow-hidden md:h-screen">
            ...
            <div class="z-0 hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{ asset('storage/' . $c->animal_image) }}"
    ...
    <div class="w-full"
        ...
        <div class="w-full h-auto"
            ...
            <div class="flex flex-col py-[40px] items-center justify-between max-w-6xl mx-auto">
                <p class="font-baloo-2-bold text-white text-[32px] mb-4">Animal Category</p>
                    ...
                    <div>
                        <div
                            ...
                            <div class="mt-[-30px] flex justify-center">
                                <a href="{{ route('animals.category', ['category' => $category->id]) }}">
                                    @if($animalCategory->id == $category->id)
                                    <button
        ...
        <!-- Detail Mamalia -->
        <div action="{{ route('search.animals') }}" method="GET" class="max-w-full">
            <div class="flex flex-col py-8 items-center justify-between max-w-6xl mx-auto">
                <div class="flex flex-row w-full justify-between items-center mb-[20px]">
                    <div class="items-center justify-center">
                        <p class="font-baloo-2-bold text-[32px]">{{ $animalCategory->name }}</p>
                    </div>
                    <div>
                        <form id="searchForm" action="{{ route('search.animals') }}" method="GET">
                            ...
                            <div class="relative w-[300px]">
                                ...
                                <input type="search" id="default-search"
                                    ...
                                    placeholder="Search hewan ..." name="query" />
                                <button type="submit"
                            ...
                            <div
                                ...
                                <div
                                    ...
                                    <img src="{{ asset('storage/' . $animal->animal_image) }}"
                                        style="width: 400px; height: 300px;" alt="{{ $animal->animal_name }}">

                                    <div class="flex flex-col justify-center gap-[30-px]">
                                        <p class="font-baloo-2-semibold text-[32px]">{{ $animal->animal_name }}</p>
                                        <p class="font-baloo-2 text-[16px]">{{ $animal->animal_desc }}</p>
                                        <a href="{{ route('animals.desc', ['id' => $animal->id]) }}">
                                            <button
                ...
                <div class="grid grid-cols-3 md:grid-cols-3 gap-5">
                    ...
                    <div class="{{ $loop->index >= 3 ? 'hidden' : '' }}">
                        <div class="h-auto max-w-full rounded-lg bg-white shadow shadow-slate-200 px-4 py-4">
                            <img src="{{ asset('storage/' . $animal->animal_image) }}" class="mb-2 rounded-lg"
                                alt="Kelinci">
                            <p class="font-baloo-2-bold text-16 text-center">{{ $animal->animal_name }}</p>
                            <p class="font-baloo-2 text-14">
                                @php
                                $truncated_desc = strlen($animal->animal_desc) > 100 ? substr($animal->animal_desc, 0,
                                100) . ' ...' : $animal->animal_desc;
                                echo $truncated_desc;
                                @endphp
                            </p>
                            <a href="{{ route('animals.desc', ['id' => $animal->id]) }}" class="block mt-2">
                                <button

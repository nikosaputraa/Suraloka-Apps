@if ($paginator->hasPages())
    <div class="flex items-center">
        ...
        @else
            <a
                class="rounded-l rounded-sm border-t border-b border-l border-brand-light px-3 py-2 text-brand-dark hover:bg-blue-600 hover:text-white no-underline"
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a class="border-t border-b border-l border-brand-light px-3 py-2 hover:bg-blue-600 hover:text-white text-brand-dark no-underline" href="{{ $url }}">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
        ...
        @if ($paginator->hasMorePages())
            <a class="rounded-r rounded-sm border border-brand-light px-3 py-2 hover:bg-blue-600 hover:text-white text-brand-dark no-underline" href="{{ $paginator->nextPageUrl() }}" rel="next">&raquo;</a>
        @else
